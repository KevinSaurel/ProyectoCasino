...
float conseguirDineroPersona(sqlite3 *db, char *nombre, char *apellido) {
    sqlite3_stmt *stmt;
    char sql[200];
    sprintf(sql, "SELECT dinero FROM Persona WHERE nombre = '%s' AND apellido = '%s'", nombre, apellido);
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        float dineroPersona = atof(sqlite3_column_text(stmt, 0));
        sqlite3_finalize(stmt);
        return dineroPersona; // Devolver una copia del dinero
    } else {
        sqlite3_finalize(stmt);
        return 0; // No se encontró el nombre
    }
}
